...
function processVtt(filename) {
  ...
  for (const block of blocks) {
    ...
    for (const line of lines) {
      ...
      if (match) {
        timeStr = line;
      } else if (!line.startsWith('WEBVTT') && !line.startsWith('Kind:') && !line.startsWith('Language:')) {
        text.push(line);
      }
    }
    if (timeStr && text.length) {
      ...
      const rawText = text.join(' ').replace(/<[^>]+>/g, '').replace(/\[âm nhạc\]/g, '').replace(/^>>\s*/, '').trim();
      if (sec >= 50 && rawText) {
        cues.push({ sec, rawText });
      }
    }
  }
  return cues;
...
const cuesSong26 = processVtt('exit_sign_song26.vi-orig.vtt');
console.log('Total Song26 CC Cues:', cuesSong26.length);
console.log(cuesSong26.slice(0, 30));
